...
def get_pp_loc_part2_atomic(cell, kpts=None):
    ...
    if all_zeros:
        ...
        for k, kpt in enumerate(kpts_lst):
            vpploc_1at_kpts = []
            for i in range(natm):
                v2 = lib.unpack_tril(buf[k,i,:])
                if abs(kpt).sum() < 1e-9:  # gamma_point:
                    v2 = v2.real
                vpploc_1at_kpts.append(v2)
            vpploc.append(vpploc_1at_kpts)
